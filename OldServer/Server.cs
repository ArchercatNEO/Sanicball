...
internal readonly record struct LogEntry(DateTime Timestamp, string Message, LogType Type);
...
internal class Server : IDisposable
{
    ...
    private readonly CharacterTier[] characterTiers = [ //Hardcoded character tiers, same reason
    ...
    private readonly Dictionary<string, string> commandHelp = [];
    ...
    private List<ServClient> clients = new();
    private List<ServPlayer> players = new();
    ...
    public Server(CommandQueue commandQueue)
    {
        ...
        cmd =>
        {
            ...
            {
                if (commandHelp.TryGetValue(cmd.Content.Trim(), out string help))
                {
                    Log(help);
                }
            }
        ...
        if (!File.Exists(CONFIG_FILENAME))
        {
            ...
            using (StreamWriter sw = new(CONFIG_FILENAME))
            {
    ...
    public void Start()
    {
        ...
        try
        {
            this.config = JsonConvert.DeserializeObject<ServerConfig>(sr.ReadToEnd());
        ...
        NetPeerConfiguration config = new(Consts.APP_ID);
...
#region done
    ...
    private void MessageLoop()
    {
        while (running)
        {
            ...
            //Check command queue
            foreach (Command cmd in commandQueue.ReadNext())
            {
            ...
            while ((msg = netServer.ReadMessage()) != null)
            {
                switch (msg.MessageType)
                {
                    ...
                    case NetIncomingMessageType.DiscoveryRequest:
                        ...
                        {
                            Config = config,
                            Players = netServer.ConnectionsCount,
                            InRace = inRace
                        };
                    ...
                    case NetIncomingMessageType.Data:
                        ...
                        switch (messageType)
                        {
                            ...
                            case MessageType.MatchMessage:
                                ...
                                {
                                    ServClient newClient = new(castedMsg.ClientGuid, castedMsg.ClientName, msg.SenderConnection);
    ...
    public void Log(object message, LogType type = LogType.Normal)
    {
        lock (log)
        {
            if (!debugMode && type == LogType.Debug)
                return;
            LogEntry entry = new(DateTime.Now, "[" + DateTime.Now.ToString("HH:mm:ss") + "] " + message.ToString(), type);
            OnLog?.Invoke(entry);
            log.Add(entry);
        }
    }
    ...
    private bool VerifyCharacterTier(int id) {
        ...
        {
            AllowedTiers.All => true,
            AllowedTiers.NormalOnly => t == CharacterTier.Normal,
            AllowedTiers.OddOnly => t == CharacterTier.Odd,
            AllowedTiers.HyperspeedOnly => t == CharacterTier.Hyperspeed,
            AllowedTiers.NoHyperspeed => t != CharacterTier.Hyperspeed,
            _ => true,
        };
    }
    ...
    private string GetAllowedTiersText() {
        return matchSettings.AllowedTiers switch
        {
            ...
            AllowedTiers.OddOnly => "Only characters from the Odd tier are allowed.",
            AllowedTiers.HyperspeedOnly => "Only characters from the Hyperspeed tier are allowed.",
            AllowedTiers.NoHyperspeed => "Any character NOT from the Hyperspeed tier is allowed.",
            _ => "All characters are allowed.",
        };
    }
    ...
    public void Dispose()
    {
        ...
        using (StreamWriter writer = new("Logs" + System.IO.Path.DirectorySeparatorChar + DateTime.Now.ToString("MM-dd-yyyy_HH-mm-ss") + ".txt"))
        {
