...
public partial class MenuCamera : Camera3D
{
    ...
    {
        if (ballMesh == null){ Log.Warning("Camera has no ball mesh: disabling mesh switching");}

        if (lightingEnv == null){ Log.Warning("Camera has no WorldEnvironment: disabling fade in/out");}
        ...
        foreach (var path in paths)
        {
            ...
            {
                Position = path.Start;
                if (ballMesh is not null) {ballMesh.MaterialOverride = path.CharacterMat;}
            }));
